...
class PairTreeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return tensorize(batch0, self.vocab, assm=False, if_need_origin_word = True),\
                 tensorize(batch1, self.vocab, assm=self.y_assm)

class MolTreeDataset(Dataset):

    def __init__(self, data, vocab, assm=True, if_need_origin_word = False):
        self.data = data
        self.vocab = vocab
        self.assm = assm
        self.if_need_origin_word = if_need_origin_word

    def __len__(self):
        return len(self.data)
    
    def __getitem__(self, idx):
        if (not self.if_need_origin_word):
            return tensorize(self.data[idx], self.vocab, assm=self.assm)
        else:
            return tensorize(self.data[idx], self.vocab, assm=self.assm,\
             if_need_origin_word=True)
