...
class DiffVAE(nn.Module):
    ...
    def forward(self, x_batch, y_batch, beta):
        x_batch, x_jtenc_holder, x_mpn_holder, origin_word = x_batch
        '''pickle.dump((x_batch, x_jtenc_holder, x_mpn_holder), open("diffvae.pkl", "wb"))
        print "save diffvae"
        exit()'''
        ...
        word_loss, topo_loss, word_acc, topo_acc = self.decoder(y_batch, x_tree_vecs, x_mol_vecs, origin_word)
    ...
    def decode(self, x_tree_vecs, x_mol_vecs, origin_word):
        ...
        pred_root,pred_nodes = self.decoder.decode(x_tree_vecs, x_mol_vecs, origin_word)
